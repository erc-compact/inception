...
class Collector:
    ...
    def load_presto_search(self, inj_dir):
        ...
        if self.c_args['presto_search']:
            ...
            for segment in segments:
                ...
                if matched_file:
                    presto_df = pd.read_csv(matched_file[0], index_col=0)
                    segment_cands.append(presto_df)
        ...
        if self.c_args['presto_candfold']:
            ...
            for segment in segments:
                ...
                if matched_file and os.path.isdir(fold_dir):
                    matched_df = pd.read_csv(matched_file[0], index_col=0)
                    psr_ids = matched_df['PSR_ID'].unique()

                    presto_candfold = cand_tools.presto_cand_bestprof2csv(psr_ids, fold_dir)
                    if len(presto_candfold):
                        presto_candfold['segment'] = segment
                        segment_cands.append(presto_candfold)
    ...
    def collect(self):
        ...
        for i, inj_dir in enumerate(inj_directories):
            ...
            presto_cands = self.load_presto_candfolds(inj_dir)
            presto_matched = self.load_presto_search(inj_dir)
            ...
            for psr in report:
                ...
                if self.c_args['presto_search']:
                    ...
                    if len(psr_presto_matched) == 0:
                        inj_results.extend(list(np.zeros(len(presto_keys))))
                    else:
                        psr_presto_matched = psr_presto_matched.sort_values(by='snr', key=abs, ascending=False)
                        psr_presto = psr_presto_matched.iloc[0]
                        inj_results.extend([len(psr_presto_matched), psr_presto['period'], psr_presto['dm'], psr_presto['F1'], psr_presto['F2'], psr_presto['snr'], psr_presto['pepoch'], psr_presto['downsample'], psr_presto['segment']])
                ...
                if self.c_args['presto_candfold']:
                    ...
                    inj_keys.extend([f'PRESTO_CAND_{key}' for key in cand_keys])
