...
class PICSScorer:
    def __init__(self,  pics_models, injection_number, out_dir, work_dir):
    ...
    def run_cmd(self):
        ...
        if len(ar_files) != 0:
            while (not os.path.exists(f'{in_path}/pics_scores.txt')) and (attempts != 3):
                attempts += 1
                cmd = f"python2 PICS_python2.py --in_path={in_path} --model_dir={self.pics_models} --work_dir={os.getcwd()}"
...
if __name__=='__main__':
    ...
    ps = PICSScorer(args.pics_models, args.injection_number, args.out_dir,  args.work_dir)
    ps.scorer_setup()
    ps.run_cmd()