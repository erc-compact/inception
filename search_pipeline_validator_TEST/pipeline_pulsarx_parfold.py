...
class PulsarxFoldParProcess:
    ...
    def run_parfold(self, psr):
        ...
        cmd = f"{fold_args['mode']} -o {tmp_cwd}/{psr_id} -f {self.data} --tsubint {tsubint} --output_width  --template {fold_args['template']} {fold_file} --blocksize {block_size} --nbin {nbins} {self.zap_string} {save_fits}"
    
        for flag in self.processing_args['pulsarx_parfold_args']['cmd_flags']:
            if key in ['output_width', 'saveimage']:
                pass
            cmd += f" {flag}"
    ...
    def transfer_products(self):
        ...
        if self.processing_args['pulsarx_parfold_args']['save_fits']:
            for pID in psr_ids:
