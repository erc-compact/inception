...
import search_pipeline_validator_TEST.candidate_tools as cand_tools
...
class CandidateMatcher:
    ...
    def setup(self):
        ...
        if self.ephem != 'builtin':
            inj_tools.rsync(self.ephem, self.work_dir)
            self.ephem = f'./{Path(self.ephem).name}'
    ...
    def parse_xml_tag(xml_tag):
        inj_ID, dir_ID, _, tscrunch,_, s0, s1 = xml_tag.split('_')
        return s0, s1, tscrunch
    ...
    def process(self):
        mode = self.processing_args['candidate_matcher']['mode']
        harmonics = self.processing_args['candidate_matcher']('harmonics', [0.5, 1, 2])
        results_dir = f'{self.out_dir}/inj_{self.injection_number:06}'
        ...
        if mode == 'PEASOUP':
            ...
            for xml in xmls:
                ...
                csv_cands['XML'] = Path(xmls).name
                s0, s1, tscrunch = self.parse_xml_tag(Path(xmls).name)
    ...
    def match_candidates(self, candidates, harmonics):
        ...
        max_folds = self.processing_args['candidate_matcher']['max_folds']
        ...
        for pm in self.setup_manager.pulsar_models:
            ...
            F_min, F_max = cand_tools.get_freq_bounds(rv_PSR, pm)
